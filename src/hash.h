...
constexpr inline uint32_t fnv1a32ext(const char* const str, const uint32_t v, const bool) {
  return *str ? fnv1a32ext(str+1, (v ^ uint8_t(*str)) * FNV1A_PRIME_32, true) : v; }
constexpr inline uint32_t fnv1a32(const char* const str) {
  return fnv1a32ext(str, FNV1A_INIT_32, true); }

constexpr inline uint32_t fnv1a32ext(const char* const str, const size_t len, const uint32_t v) {
  return len ? fnv1a32ext(str+1, len-1, (v ^ uint8_t(*str)) * FNV1A_PRIME_32) : v; }
constexpr inline uint32_t fnv1a32(const char* const str, const size_t len) {
  return fnv1a32ext(str, len, FNV1A_INIT_32); }

constexpr inline uint64_t fnv1a64ext(const char* const str, const uint64_t v, const bool) {
  return *str ? fnv1a64ext(str+1, (v ^ uint8_t(*str)) * FNV1A_PRIME_64, true) : v; }
constexpr inline uint64_t fnv1a64(const char* const str) {
  return fnv1a64ext(str, FNV1A_INIT_64, true); }

constexpr inline uint64_t fnv1a64ext(const char* const str, const size_t len, const uint64_t v) {
  return len ? fnv1a64ext(str+1, len-1, (v ^ uint8_t(*str)) * FNV1A_PRIME_64) : v; }
constexpr inline uint64_t fnv1a64(const char* const str, const size_t len) {
  return fnv1a64ext(str, len, FNV1A_INIT_64); }
